    ...
    try {
      await _speedTestPlugin.startTesting(
        ...
        onDefaultServerSelectionDone: (Client? client) {
          if (client != null) {
            debugPrint(
              'Server selected - IP: ${client.ip}, ISP: ${client.isp}, Country: ${client.location}',
            );
          } else {
            debugPrint('Using default server');
          }
        ...
        onDownloadComplete: (TestResult data) {
          ...
          debugPrint(
            'Download complete: ${data.transferRate} ${data.unit.name}',
          );
        },
        ...
        onProgress: (double percent, TestResult data) {
          ...
          if (data.type == TestType.download) {
            ...
            debugPrint(
              'Download progress: $percent% - ${data.transferRate} ${data.unit.name}',
            );
          } else if (data.type == TestType.upload) {
            testPhase.value = 'upload';
            uploadProgress.value = percent;
            debugPrint(
              'Upload progress: $percent% - ${data.transferRate} ${data.unit.name}',
            );
          }
        },
        onCompleted: (TestResult download, TestResult upload) {
          ...
          debugPrint(
            'Download: ${download.transferRate} ${download.unit.name}',
          );
